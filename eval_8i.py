...
def parse_args():
  ...
  # For 8x upsampling
  parser.add_argument("--pretrained", default='./ckpts/8x_0x_ks7/iter64000.pth', help='Path to pretrained model')
  # For 4x upsampling
  ...
  parser.add_argument("--test_dataset", default='../Data/8i_test/8x/longdress/Ply/')
  parser.add_argument("--save_loc", default='results_8i/')
...
for i, pc in enumerate(test_files):
    ...
    for j,part in enumerate(parts):
        ...
        x = ME.SparseTensor(features=f.to(device), coordinates=p.to(device))
    ...
    rec_pc_cpu = rec_pc.cpu().numpy()
    rec_pcd.points = o3d.utility.Vector3dVector(rec_pc_cpu)
    print("Number of points in output rec point cloud : ", rec_pc_cpu.shape[0])
